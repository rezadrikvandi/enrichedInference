...
enrichedInference_oldversion <- function(X, Y, k=1, intercept=TRUE, standardize=FALSE, CVlasso=FALSE, Xcorr=c("marginal","partial"), sig.level=0.05, Bonf.correction=FALSE)
{
  Y.train  <- as.vector(Y);
  X.train <- as.matrix(X);
  nn <- nrow(X.train);
  ...
  if(Xcorr=="partial")
  {
    if(ncol(Xu)>2000)
    {
      CXu <- apply(Xu, 2, function(a) (t(a)%*%Y.train/sqrt(t(a)%*%a)));
      rank_CXu <- rank(-abs(as.vector(CXu)));
      remove_Xu <- which(rank_CXu>2000);
      Xu <- Xu[,-remove_Xu];
    }
    pcorr <- suppressWarnings(try(pcor(cbind(Y.train,Xu)),silent=TRUE));
    Xu <- Xu[,-which(c(pcorr$estimate[1,])[-1]<0.05)];
  }
  if(Xcorr=="marginal")
  {
    CXu <- apply(Xu, 2, function(a) (t(a)%*%Y.train/sqrt(t(a)%*%a)));
    ...
    if(ncol(Xu)>100)
    {
      remove_Xu <- which(rank_CXu>length(rank_CXu)*0.5);
      Xu <- Xu[,-remove_Xu];
    }
  }
  q <- ncol(Xs);
